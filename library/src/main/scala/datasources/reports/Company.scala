...
/** represents a company-info document
 */
case class Company(override val file:File) extends TextReport(file) with StrictLogging {
  val FORMS = S1 :: Nil
  // S-1 keys
  val SIC_KEY = "STANDARD INDUSTRIAL CLASSIFICATION:"
  val ISTATE_KEY = "STATE OF INCORPORATION:"
  
  override lazy val uid = cik
  
  lazy val sic = {
	  logger.info("Parsing S-1 form: "+file)
	  def parseSIC(line:String) = line slice (line.indexOf("[") + 1, line.indexOf("]"))

	  val lines = Source.fromFile(file).getLines
    lines find (_ contains SIC_KEY) map parseSIC getOrElse ""
  }
  
  def factTransform = {
    val industry = Try(sic.toInt) map (i => 
      Fact("SIC", "industry", FactString(sic), Seq(Company.lookupSIC(i)))
    )
...
/** TODO maybe this list should be in a separate file
 */
object Company {
  
  def lookupSIC(sic:Int) = sicMap get sic getOrElse ""
